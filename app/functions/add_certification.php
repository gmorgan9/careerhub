...
if (isset($_POST['add-certification'])) {
    ...
    if(isset($_POST['cert_status'])) { $cert_status = mysqli_real_escape_string($conn, $_POST['cert_status']); } else { $cert_status = ""; }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $insert = "INSERT INTO certifications (idno, cert_name, cert_issued, cert_expire, cert_renewed, cert_status, cert_provider, cert_recipient, cert_tags, notes) VALUES ('$idno', NULLIF('$cert_name',''), NULLIF('$cert_issued',''), NULLIF('$cert_expire',''), NULLIF('$cert_renewed',''), NULLIF('$cert_status',''), NULLIF('$cert_provider',''), NULLIF('$cert_recipient',''), NULLIF('$cert_tags',''), NULLIF('$notes',''))";
